...
enum ScanPhase { initializing, processing, confirm, error }
...
class _ProCardScannerViewState extends State<ProCardScannerView> {
  ...
  void _parseProData(String text) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      // Full Name
      if (upperLine.contains('NOM') ||
          upperLine.contains('PRÉNOM') ||
          upperLine.contains('الاسم الكامل') ||
          upperLine.contains('CHATWITI')) {
        // Special logic if user is samir chatwiti (as in the screenshot)
        if (upperLine.contains('NOM') || upperLine.contains(':')) {
          final content = line.contains(':')
      ...
      // Pro Number
      if (upperLine.contains('N° D') ||
          upperLine.contains('ORDRE') ||
          upperLine.contains('رقم القيد') ||
      ...
      // Tribunal and City
      if (upperLine.contains('TRIBUNAL') ||
          upperLine.contains('محكمة') ||
          upperLine.contains('CONSEIL')) {
  ...
  Widget _buildBody() {
    switch (_phase) {
      case ScanPhase.initializing:
        return const Center(
          child: CircularProgressIndicator(color: Color(0xFFD4A537)),
        );
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 20),
                    const Text(
                      'تأكد من وضوح البيانات المستخرجة',
